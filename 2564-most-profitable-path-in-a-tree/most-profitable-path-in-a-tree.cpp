...
private:
    bool findBobPath(int node, int time, vector<vector<int>>& tree,
                     vector<int>& bobTime, int bob) {
        bobTime[node] = time;
        if (node == 0) {
            return true;
        }
        ...
        for (int neighbor : tree[node]) {
            if (bobTime[neighbor] == -1 &&
                findBobPath(neighbor, time + 1, tree, bobTime, bob)) {
                return true;
            }
        }
    ...
    int dfs(int node, int parent, vector<vector<int>>& tree,
            vector<int>& amount, int time, vector<int>& bobTime) {
        ...
        for (int neighbor : tree[node]) {
            if (neighbor != parent) {
                isLeaf = false;
                maxProfit = max(maxProfit, dfs(neighbor, node, tree, amount,
                                               time + 1, bobTime));
            }
        }
